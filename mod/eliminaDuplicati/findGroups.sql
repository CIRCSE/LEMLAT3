...
HAVING (NoD=1 AND NoLemLat>1 OR NoD>1) AND NoLiLaH+NoLiLaL=1
       ...
       AND lemma NOT IN ('cardo', 'inlex', 'ales', 'lar', 'merges', 'sanguis', 'uis')
       AND NOT (lemma='mas' AND codlem='N3B')
;
...
WHERE (NoB=1 OR NoB=0 AND NoO=1 OR NoB=0 AND NoO=0 ) AND LEFT(lm.n_id,1)<>'$' AND keep.n_id IS NULL;
...
REV3:
- lemma che finisce in -us (codlem=N2) ->
  se la differenza tra B e D/O è la presenza di un qualsiasi valore nei campi a_gra e/o lem in B e/o la sua assenza in D/O,
  si proceda pure alla eliminazione del lemma/i di D/O
- lemma che finisce in -is (codlem=N3B) ->
  se la differenza tra B e D/O è che sulla riga di B il codles=n31 OR codles=n32 e sulla riga di D/O il codles=n3 (OR codles=n3p),
  si proceda pure alla eliminazione del lemma/i di D/O,
  ANCHE SE CI SONO DIFFERENZE IN ALTRE COLONNE (di solito, lem e codLE)
- lemma che finisce in -er (codlem=N3B) ->
  se la differenza tra B e D/O è il valore di codles (posto che inizi in entambi i casi con n3*),
  si proceda pure alla eliminazione del lemma/i di D/O, ANCHE SE CI SONO DIFFERENZE IN ALTRE COLONNE
- se i lemmi in questione hanno codlem=N3A,
  procedere alla eliminazione, tranne nel caso di "inlex", su cui non bisogna fare niente
- lemma che finisce in -o (codlem=N3B) -> procedere alla eliminazione, tranne nel caso di D314B, che va mantenuto
- lemma che finisce in -a (codlem=N1) -> procedere alla eliminazione

REV5:
- lemma con codlem=PR -> si proceda pure alla eliminazione del lemma/i candidato/i
- lemma con codlem=N* -> si proceda pure alla eliminazione del lemma/i candidato/i,
tranne nei seguenti casi: ales, lar (3 lemmi), mas, merges. Nel caso di "sanguis",
il lemma da tenere è DCD0F (invece di DCD0D): tutti gli altri sono da eliminare.
...
CREATE TEMPORARY TABLE checkMerge
   ...
   FROM (
        ...
        FROM keep K INNER JOIN lessario ls USING(n_id)
              ...
              AND (
                   -- NB: incondizionata (dalle altre colonne)
                   k.codlem REGEXP '^N.*'
                OR
                   k.codles IN ('n31', 'n32') AND t.codles IN ('n3n','n3p') AND k.codlem='N3B' AND k.lemma REGEXP '.+is$'
                OR
                   k.codles REGEXP '^n3.*' AND t.codles REGEXP '^n3.*' AND k.codlem='N3B' AND k.lemma REGEXP '.+er$'
                OR
                   k.codlem='N3A'
                OR
                   k.codlem='N3B' AND k.lemma REGEXP '.+o$'
                OR
                   k.codlem='N1' AND k.lemma REGEXP '.+a$'
                OR
                   k.codlem IN ('N1', 'N4', 'I', 'PR')
                OR REPLACE(k.les,'h','')=REPLACE(t.les,'h','') AND (
                      k.codlem='N2' AND k.codles REGEXP '^n2.*' AND t.codles REGEXP '^n2.*'
                   OR
                      k.codlem='N2/1' AND k.codles REGEXP '^n6.*' AND t.codles REGEXP '^n6.*'
                   OR
                      k.codlem='N3B' AND k.codles REGEXP '^n3.*' AND t.codles REGEXP '^n3.*'
                   )
                OR    
              k.ls_gen=t.ls_gen
              ...
              AND (k.a_gra=t.a_gra OR t.a_gra=''
                  OR k.a_gra<>'' AND t.a_gra='' AND k.codlem IN ('N2/1','N2') AND k.lemma REGEXP '.+us$'
