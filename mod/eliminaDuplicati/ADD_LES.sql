...
SELECT DISTINCT n_id_Keep, ls_gen gen, clem, si, smv, spf, les, codles, lem, piu, ls_codlem codlem, type, codLE, pt, a_gra, gra_u, src 
FROM
(
...
(
   ...
   FROM checkMerge INNER JOIN (
        SELECT T.*, pr_key,
               ls.gen ls_gen, clem, si, smv, spf, les, codles, lem, piu, ls.codlem ls_codlem, type, codLE, pt, a_gra, gra_u, src
...
UPDATE
( SELECT DISTINCT n_id_Keep n_id FROM addLes ) A
INNER JOIN lessario L USING(n_id)
INNER JOIN (
SELECT I.*,n_id, SUM(L.clem='v') NoV, COUNT(*) Tot
FROM checkMerge I INNER JOIN keep K USING(lemma, codlem, gen, codmorf) INNER JOIN lessario L USING(n_id)
GROUP BY lemma, codlem, gen, codmorf, n_id
...
SELECT n_id_Keep, gen, clem, si, smv, spf, les, codles, lem, piu, codlem, type, codLE, pt, a_gra, gra_u, src 
FROM addLes;
