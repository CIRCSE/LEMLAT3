...
create-version=1
source=select `lemmario`.`id_lemma` AS `id_lemma`,`lemmario`.`lemma` AS `lemma`,`lemmario`.`codlem` AS `codlem`,`lemmario`.`gen` AS `gen`,`lemmario`.`codmorf` AS `codmorf`,`lemmario`.`n_id` AS `n_id`,`lemmario`.`lemma_reduced` AS `lemma_reduced`,`lemmario`.`src` AS `src` from `lemmario` where (`lemmario`.`src` = \'O\')
client_cs_name=utf8
